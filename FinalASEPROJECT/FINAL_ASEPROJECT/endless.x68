...
PLYR_DFLT_G EQU         11          ; Player Default Gravity

GND_TRUE    EQU         01          ; Player on Ground True
GND_FALSE   EQU         01          ; Player on Ground False
...
ENMY2_W_INIT EQU       08          ; Enemy2 initial Width
ENMY2_H_INIT EQU       08          ; Enemy2 initial Height

BULLET_W_INIT EQU        450          ; Heal initial Width
BULLET_H_INIT EQU        08          ; Heal initial Height
...
INITIALISE:
    ...
    DIVU    #04,        D1          ; divide by 4 for DROPPING the player on the center 
    ...
    MOVE.L  D2,         BULLET_X     ; BULLET X Position

    CLR.L   D2                      ; Clear contents of D1 (XOR is faster)
    MOVE.W  SCREEN_H,   D2          ; Place Screen width in D1
    DIVU    #03,        D2          ; divide by 2 for center on Y Axis
    MOVE.L  D2,         BULLET_Y     ; BULLET Y Position
...
GAMELOOP:
    ...
    BSR     CHECK_COLLISIONS2       ; Check for Collisions2
    BSR     CHECK_COLLISIONS3       ; Check for Collisions3
...
UPDATE:
    ...
    CLR.L   D1                      ; Clear contents of D1 (XOR is faster)
    CLR.L   D1                      ; Clear the contents of D0
    MOVE.L  ENEMY2_X,    D1          ; Move the Enemy2 X Position to D0
    CMP.L   #00,        D1
...
MOVE_ENEMY:
    ...
    SUB.L   #01,        ENEMY2_X    ; Move enemy2 by X Value
    
    SUB.L   #01,        BULLET_X    ; Move enemy3 by X Value
...
GAME_LOOP:
    ...
    CMP.L   #2, D3      
    BLE     GAME_LOOP    ; WE NEED TO DELAY A BIT LONGER
    MOVE.L  D4, D2       ; SET PREVIOUS TIME TO CURRENT TIME
    
    SUB.L   #02,        ENEMY_X     ; Move enemy by X Value
    SUB.L   #01,        ENEMY2_X    ; Move enemy2 by X Value
    
    SUB.L   #03,        BULLET_X    ; Move enemy3 by X Value
    
    RTS

*-----------------------------------------------------------
* Subroutine    : Reset Enemies
* Description   : Reset Enemies if to passes 0 to Right of Screen
*-----------------------------------------------------------
...
RESET_ENEMY2_POSITION:
    CLR.L   D1                      ; Clear contents of D2 (XOR is faster)
    MOVE.W  SCREEN_W,   D1          ; Place Screen width in D2
    MOVE.L  D1,         ENEMY2_X    ; Enemy2 X Position
    
RESET_PLUS_POSITION:
    CLR.L   D1                      ; Clear contents of D2 (XOR is faster)
    MOVE.W  SCREEN_W,   D1          ; Place Screen width in D2
    MOVE.L  D1,         BULLET_X    ; BULLET X Position
    RTS
...
* Description   : Draw BULLET enemy3
*-----------------------------------------------------------    
...
DRAW_BULLET:
    ...
    MOVE.L  BULLET_X,    D3
    ADD.L   #BULLET_W_INIT,   D3      ; Width
    MOVE.L  BULLET_Y,    D4 
    ADD.L   #BULLET_H_INIT,   D4      ; Height
    
    ; Draw PLUS    
    MOVE.B  #87,        D0          ; Draw BULLET
    TRAP    #15                     ; Trap (Perform action)
    RTS

*-----------------------------------------------------------
* Subroutine    : Draw Enemies
...
PLAYER_X_LTE_TO_ENEMY2_X_PLUS_W:
    MOVE.L  PLAYER_X,    D1          ; Move Player X to D1
    MOVE.L  ENEMY2_X,    D2          ; Move Enemy2 X to D2
...
PLAYER_Y_LTE_TO_ENEMY2_Y_PLUS_H:     
    MOVE.L  PLAYER_Y,    D1                  ; Move Player Y to D1
    MOVE.L  ENEMY2_Y,    D2                  ; Move Enemy2 Y to D2
...
PLAYER_Y_PLUS_H_LTE_TO_ENEMY2_Y:     ; Less than or Equal ?
    ADD.L   PLYR_H_INIT,D1           ; Add Player Height to D1
    MOVE.L  ENEMY2_Y,   D2           ; Move Enemy2 Height to D2  
    CMP.L   D1,         D2           ; Do they OverLap ?
...
PLAYER_X_PLUS_W_LTE_TO_BULLET_X:     ; Check player is not  
    ADD.L   BULLET_W_INIT,D1         ; Move Player Width to D1
    MOVE.L  BULLET_X,    D2          ; Move BULLET X to D2
    CMP.L   D1,         D2           ; Do they OverLap ?
    BGE     PLAYER_Y_LTE_TO_BULLET_Y_PLUS_H  ; Less than or Equal
    BRA     COLLISION_CHECK_DONE3     ; If not no collision

      
PLAYER_X_LTE_TO_BULLET_X_PLUS_W:
    MOVE.L  PLAYER_X,   D1           ; Move Player X to D1
    MOVE.L  BULLET_X,    D2          ; Move BULLET X to D2
    ADD.L   BULLET_W_INIT,D2         ; Set BULLET width X + Width
    CMP.L   D1,         D2           ; Do the Overlap ?
    BLE     PLAYER_X_PLUS_W_LTE_TO_BULLET_X  ; Less than or Equal ?
    BRA     COLLISION_CHECK_DONE3     ; If not no collision 

    
PLAYER_Y_LTE_TO_BULLET_Y_PLUS_H:     
    MOVE.L  PLAYER_Y,   D1           ; Move Player Y to D1
    MOVE.L  BULLET_Y,    D2          ; Move BULLET Y to D2
    ADD.L   BULLET_H_INIT,D2         ; Set BULLET Height to D2
    CMP.L   D1,         D2           ; Do they Overlap ?
    BLE     PLAYER_Y_PLUS_H_LTE_TO_BULLET_Y  ; Less than or Equal
    BRA     COLLISION_CHECK_DONE3     ; If not no collision 
    
    
  
PLAYER_Y_PLUS_H_LTE_TO_BULLET_Y:    ; Less than or Equal ?
    ADD.L   PLYR_H_INIT,D1          ; Add Player Height to D1
    MOVE.L  BULLET_Y,   D2          ; Move BULLET Height to D2  
    CMP.L   D1,         D2          ; Do they OverLap ?
...
BULLET_X          DS.L    01  ; Reserve Space for Bullet X Position
BULLET_Y          DS.L    01  ; Reserve Space for Bullet Y Position
